...
public class MyList<Integer> extends ArrayList<Integer> {
    ...
    @Override
    public boolean addAll(Collection<? extends Integer> arr) {
        for (Integer i : arr) {
            this.add(i);
        }
        return true;
    ...
    @Override
    public boolean addAll(int idx, Collection<? extends Integer> arr) {
        int index = idx;
        for (Integer i : arr) {
            this.add(index-1, i);
            index++;
    ...
    public boolean remove(Object el) {
        boolean res = false;
        res = super.remove(el);
        if (this.contains((Integer)el) == false)
            this.differentElements--;
        return res;
    }
